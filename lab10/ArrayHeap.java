...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        // Throws an exception if index is invalid. DON'T CHANGE THIS LINE.
        validateSinkSwimArg(index);
        int childMinIndex = min(leftIndex(index), rightIndex(index));
        while (index < size && childMinIndex == min(childMinIndex, index)) {
            swap(index, childMinIndex);
            index = childMinIndex;
            childMinIndex = min(leftIndex(index), rightIndex(index));
        }
    }
    ...
    public T removeMin() {
        ...
        swap(1, size);
        T itemRemoved = contents[size].item();
        contents[size] = null;
        size -= 1;
        if (size > 1) {
            sink(1);
